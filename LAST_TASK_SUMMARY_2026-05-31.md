...
Táto vlastnosť je označená ako runtime-only (`[JsonIgnore]`) a slúži na živé zobrazenie aktívneho/obsadeného stavu kontaktového indikátora bez zásahu do serializovaných projektových dát.
...
Tým vznikla jednotná vrstva, cez ktorú vie konkrétny klient (napr. `Z21Client`) publikovať zmeny spätnoväzbových vstupov a vyššie vrstvy ich môžu spracovať bez priameho závisenia od implementácie klienta.
...
V `Z21Client` som upravil registráciu `LAN_SET_BROADCASTFLAGS` tak, aby sa k existujúcim flagom doplnil aj feedback flag pre R-Bus / S88:
- predtým: `0x00000101`
- teraz: `0x00000111`
