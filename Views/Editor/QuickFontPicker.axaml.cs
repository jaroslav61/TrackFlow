...
public partial class QuickFontPicker : Window
{
    ...
    public QuickFontPicker(string currentFont, double currentSize) : this()
    {
        ...
        if (fontSizeInput != null) fontSizeInput.Value = (decimal)currentSize;
    }
    ...
    private void Ok_Click(object? sender, RoutedEventArgs e)
    {
        ...
        if (fontListBox?.SelectedItem is ListBoxItem selectedItem) SelectedFont = selectedItem.Content?.ToString();

        if (fontSizeInput != null) SelectedSize = (double)(fontSizeInput.Value ?? 12m);
